...
// ─── Backup / restore ──────────────────────────────────────────────────────
// Raw-row access, used only by services/backup. Everything else in the app
// should go through the mapped helpers above.

/**
 * Every stored column for the given user, oldest first.
 *
 * Deliberately returns raw rows rather than StoredTrip: a backup has to be
 * lossless, and rowToTrip drops `user_uid` / `updated_at`, which are exactly
 * the fields restore needs to attribute a trip and resolve conflicts.
 */
export async function exportTripRows(params: {
  email?: string | null;
  uid?: string | null;
}): Promise<TripRow[]> {
  const { email, uid } = params;
...
export interface ImportResult {
  added: number;
  updated: number;
  /** Local copy was the same age or newer, so it was left alone. */
  skipped: number;
  /** Row was missing required fields and could not be trusted. */
  invalid: number;
}
...
/**
 * Merges backed-up rows into the local table. Newest `updated_at` wins and
 * nothing is ever deleted, so restoring an old backup onto a device that has
 * newer trips cannot destroy them — the failure mode of a naive
 * "wipe then insert" restore.
 */
export async function importTripRows(rows: unknown[]): Promise<ImportResult> {
  const db = await getDb();
  const result: ImportResult = { added: 0, updated: 0, skipped: 0, invalid: 0 };

  for (const candidate of rows) {
    const row = candidate as Partial<TripRow>;

    if (
      !row ||
      !isNonEmptyString(row.doc_id) ||
      !isNonEmptyString(row.trip_plan) ||
      !isNonEmptyString(row.trip_data)
    ) {
      result.invalid++;
      continue;
    }
    ...
    const createdAt =
      typeof row.created_at === "number" ? row.created_at : incomingUpdatedAt || Date.now();

    const existing = await db.getFirstAsync<{ updated_at: number }>(
      "SELECT updated_at FROM trips WHERE doc_id = ?;",
      [row.doc_id]
    );
    ...
    await db.runAsync(
      ...
      [
        ...
        row.start_date ?? null,
        row.end_date ?? null,
        typeof row.total_days === "number" ? row.total_days : null,
        row.budget ?? null,
        row.traveler_type ?? null,
        row.is_free_trip === 1 ? 1 : 0,
        createdAt,
